...
class HttpRequests {
 public:
  ...
  bool Post(const char* lpUrl, unsigned char* lpData,
            unsigned int nSize);  // Post字符串或二进制
 ...
 protected:
  static size_t WriteCallback(void* pBuffer, size_t nSize, size_t nMemByte,
                              void* pParam) {
    HttpRequests* h_requests = (HttpRequests*)pParam;
    size_t dwriten = 0;
    if (h_requests == nullptr) return dwriten;
    switch (h_requests->m_Flag) {
      case R_Download: {
        if (h_requests->fp == nullptr) return dwriten;
        if (!fwrite(pBuffer, nSize, nMemByte, h_requests->fp)) return dwriten;
      } break;
      case R_Post:
      case R_Get: {
  ...
  // static size_t   HeaderCallback(void* pBuffer, size_t nSize, size_t
  // nMemByte, void* pParam);
  static int ProgressCallback(void* pParam, double dltotal, double dlnow,
                              double ultotal, double ulnow) {
    HttpRequests* h_requests = (HttpRequests*)pParam;
    if (h_requests != nullptr) {
      h_requests->m_Callback->Progress(h_requests->m_pCallbackParam, dltotal,
                                       dlnow);
    }
    return 0;
  }
