...
static void* thread_routine(void *arg)
{
	...
	while(1)
	{
		...
		while(T_Pool->pool->cur_queue_size ==0 && !T_Pool->pool->shutdown)
		{
			...
			pthread_cond_wait(&(T_Pool->pool->queue_ready), &(T_Pool->pool->queue_lock));
#endif	
...
#ifdef DEBUG
			LEANO_LOG_INFO << "thread " << (long long unsigned int)pthread_self() <<" will exit\n";
#endif	
			pthread_exit(NULL);
		}
...
//#ifdef DEBUG
//		LEANO_LOG_INFO << "thread " << (long long unsigned int)pthread_self() << " is start to work....\n";		
//#endif	
