...
namespace seetaas
{
	...
	bool HttpRequests::Post(const char* lpUrl, unsigned char* lpData, unsigned int nSize)
	{
		if(lpUrl == nullptr || lpData == nullptr || nSize < 0 || m_pCurl == nullptr)
		{
			LEANO_LOG_ERROR << "Post wrong";
			return false;
		}
	...
	const string& HttpRequests::GetRespons()const
	{
		return m_Respons_str;
	}

	
	/*size_t WriteCallback(void* pBuffer, size_t nSize, size_t nMemByte, void* pParam)
	{
	...
	/*int ProgressCallback(void *pParam, double dltotal, double dlnow, double ultotal, double ulnow)
	{
		HttpRequests* h_requests = (HttpRequests*)pParam;
		if(h_requests != nullptr)
		{
